...
int HashMgr::add_hidden_capitalized_word(char *word, int wbl, int wcl,
  ...
  if (((captype == HUHCAP) || (captype == HUHINITCAP) ||
    ...
    unsigned short *flags2 =
        (unsigned short *)malloc(sizeof(unsigned short) * (al + 1));
    if (!flags2)
      return 1;
    if (al)
...
int HashMgr::add_with_affix(const char *word, const char *example) {
  ...
  if (dp && dp->astr) {
    ...
    if (aliasf) {
      ...
      unsigned short *flags =
          (unsigned short *)malloc(dp->alen * sizeof(short));
      if (flags) {
...
int HashMgr::load_tables(const char *tpath, const char *key) {
  ...
  if (strncmp(ts, "\xEF\xBB\xBF", 3) == 0) {
    ...
    HUNSPELL_WARNING(stderr, "warning: dic file begins with byte order mark: "
                             "possible incompatibility with old Hunspell "
                             "versions\n");
  }

  if ((*ts < '1') || (*ts > '9'))
    HUNSPELL_WARNING(stderr, "error - missing word count in dictionary file\n");
  tablesize = atoi(ts);
  if (!tablesize) {
    delete dict;
    return 4;
...
int HashMgr::decode_flags(unsigned short **result, char *flags) {
  ...
  case FLAG_LONG: { // two-character flags (1x2yZz -> 1x 2y Zz)
    ...
    if (len % 2 == 1)
      HUNSPELL_WARNING(
          stderr, "error: length of FLAG_LONG flagvector is odd: %s\n", flags);
    len /= 2;
  ...
  case FLAG_NUM: { // decimal numbers separated by comma (4521,23,233 -> 4521 23
                   // 233)
    len = 1;
    char *src = flags;
    unsigned short *dest;
...
int HashMgr::load_config(const char *affpath, const char *key) {
  ...
  while ((line = afflst->getline())) {
    ...
    if ((strncmp(line, "FLAG", 4) == 0) && isspace(line[4])) {
      ...
      if (flag_mode == FLAG_CHAR) {
        HUNSPELL_WARNING(
            stderr, "error: FLAG need `num', `long' or `UTF-8' parameter: %s\n",
            line);
      }
    }
...
int HashMgr::parse_aliasf(char *line, FileMgr *af) {
  if (numaliasf != 0) {
    HUNSPELL_WARNING(
        stderr, "error: duplicate AF (alias for flag vector) tables used\n");
    return 1;
  }
  char *tp = line;
  char *piece;
...
int HashMgr::is_aliasf() { return (aliasf != NULL); }
...
int HashMgr::parse_aliasm(char *line, FileMgr *af) {
  if (numaliasm != 0) {
    HUNSPELL_WARNING(stderr, "error: duplicate AM (aliases for morphological "
                             "descriptions) tables used\n");
    return 1;
  }
  char *tp = line;
  char *piece;
...
int HashMgr::is_aliasm() { return (aliasm != NULL); }
