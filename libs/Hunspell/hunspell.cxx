...
int Hunspell::cleanword2(char *dest, const char *src, w_char *dest_utf, int *nc,
...
int Hunspell::cleanword(char *dest, const char *src, int *pcaptype,
                        int *pabbrev) {
  unsigned char *p = (unsigned char *)dest;
  const unsigned char *q = (const unsigned char *)src;
  int firstcap = 0;
...
hentry *Hunspell::spellsharps(char *base, char *pos, int n, int repnum,
                              char *tmp, int *info, char **root) {
  pos = strstr(pos, "ss");
  if (pos && (n < MAXSHARPS)) {
    *pos = '\xC3';
    *(pos + 1) = '\x9F';
...
int Hunspell::spell(const char *word, int *info, char **root) {
  ...
  // allow numbers with dots and commas (but forbid double separators: "..",
  // ",," etc.)
  ...
  if (langnum == LANG_hu) {
    if ((nstate == NNUM) &&
        ((cw[i] == '%') ||
         ((!utf8 && (cw[i] == '\xB0')) ||
          (utf8 &&
           (strncmp(cw + i, "\xC2\xB0", 2) == 0 ||      // UTF-8 degree
            strncmp(cw + i, "\xE2\x80\xB0", 3) == 0)))) // UTF-8 per mille
        &&
        checkword(cw + i, info, root))
      return 1;
  }
    ...
    if (dash) {
      *dash = '\0';
      ...
      if (dash[1] == '\0') { // base word ending with dash
        ...
        dash[1] = r2;
        dash[0] = '\0';
        if (result && spell(dash + 1) &&
            ((strlen(dash + 1) > 1) || (dash[1] == 'e') ||
             ((dash[1] > '0') && (dash[1] < '9'))))
          return 1;
      }
...
struct hentry *Hunspell::checkword(const char *w, int *info, char **root) {
  ...
  if (!he && pAMgr) {
    ...
    if (he) {
      ...
      he = pAMgr->compound_check(word, len, 0, 0, 100, 0, NULL, 0, NULL, NULL,
                                 0);
      ...
      if ((!he) && (langnum == LANG_hu) && (word[len - 1] == '-')) {
        ...
        if (!dup)
          return NULL;
        dup[len - 1] = '\0';
        he = pAMgr->compound_check(dup, len - 1, -5, 0, 100, 0, NULL, 1, NULL,
                                   NULL, 0);
        free(dup);
      }
...
void Hunspell::free_list(char ***slst, int n) { freelist(slst, n); }

char *Hunspell::get_dic_encoding() { return encoding; }
...
const char *Hunspell::get_wordchars() { return pAMgr->get_wordchars(); }
...
const char *Hunspell::get_version() { return pAMgr->get_version(); }
  ...
  *result = '\0';
  ...
  if ((langnum == LANG_hu) && dash) {
    *dash = '\0';
    ...
    if (dash[1] == '\0') { // base word ending with dash
      if (spell(cw))
        return line_tok(pSMgr->suggest_morph(cw), slst, MSEP_REC);
...
int Hunspell::spellml(char ***slst, const char *word) {
  ...
  if (check_xml_par(q, "type=", "analyze")) {
    int n = 0, s = 0;
    if (get_xml_par(cw, strchr(q2, '>'), MAXWORDUTF8LEN))
      n = analyze(slst, cw);
    if (n == 0)
      return 0;
...
void Hunspell_destroy(Hunhandle *pHunspell) { delete (Hunspell *)(pHunspell); }
