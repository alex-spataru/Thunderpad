...
AffixMgr::AffixMgr(const char *affpath, HashMgr **ptr, int *md,
                   const char *key) {
  ...
  checkcompoundrep = 0; // forbid bad compounds (may be non compound word with a
                        // REP substitution)
  checkcompoundcase =
...
AffixMgr::~AffixMgr() {
  ...
  if (maptable) {
    for (int j = 0; j < nummap; j++) {
      if (maptable[j].set)
        free(maptable[j].set);
      if (maptable[j].set_utf16)
        free(maptable[j].set_utf16);
      maptable[j].set = NULL;
      maptable[j].len = 0;
    }
    free(maptable);
...
int AffixMgr::parse_file(const char *affpath, const char *key) {
  ...
  while ((line = afflst->getline())) {
    ...
    if (firstline) {
      ...
      if (strncmp(line, "\xEF\xBB\xBF", 3) == 0) {
        ...
        HUNSPELL_WARNING(stderr, "warning: affix file begins with byte order "
                                 "mark: possible incompatibility with old "
                                 "Hunspell versions\n");
      }
    }
  ...
  if (!utf8) {
    ...
    for (int i = 0; i <= 255; i++) {
      if ((csconv[i].cupper != csconv[i].clower) && (!strchr(expw, (char)i))) {
...
int AffixMgr::cpdrep_check(const char *word, int wl) {
  ...
  for (int i = 0; i < numrep; i++) {
    ...
    while ((r = strstr(r, reptable[i].pattern)) != NULL) {
      ...
      if (r - word + lenr + strlen(r + lenp) >= MAXLNLEN)
        break;
      strcpy(candidate + (r - word), reptable[i].pattern2);
      strcpy(candidate + (r - word) + lenr, r + lenp);
      if (candidate_check(candidate, strlen(candidate)))
        return 1;
...
int AffixMgr::defcpd_check(hentry ***words, short wnum, hentry *rv,
                           hentry **def, char all) {
  signed short
...
struct hentry *AffixMgr::compound_check(
    const char *word, int len, short wordnum, short numsyllable,
    short maxwordnum, short wnum, hentry **words = NULL, char hu_mov_rule = 0,
    int *cmpdstemnum = NULL, int *cmpdstem = NULL, char is_sug = 0) {
  int i;
  ...
  for (i = cmin; i < cmax; i++) {
    ...
    if (!rv) {
      if (compoundflag &&
        ...
        if ((rv = suffix_check(st, i, 0, NULL, NULL, 0, NULL, FLAG_NULL,
            ...
            !hu_mov_rule && ((SfxEntry *)sfx)->getCont() &&
            ((compoundforbidflag &&
              TESTAFF(((SfxEntry *)sfx)->getCont(), compoundforbidflag,
                      ((SfxEntry *)sfx)->getContLen())) ||
             (compoundend && TESTAFF(((SfxEntry *)sfx)->getCont(), compoundend,
                                     ((SfxEntry *)sfx)->getContLen())))) {
          rv = NULL;
        }
      }
    ...
    if ((rv) && !checked_prefix && (wordnum == 0) && compoundmiddle &&
        !hu_mov_rule && ((pfx && ((PfxEntry *)pfx)->getCont() &&
    ...
    if (((rv) &&
        ...
        ((!rv) && (langnum == LANG_hu) && hu_mov_rule &&
         ...
         (sfx && ((SfxEntry *)sfx)->getCont() &&
          ( // XXX hardwired Hungarian dic. codes
              TESTAFF(((SfxEntry *)sfx)->getCont(), (unsigned short)'x',
                      ((SfxEntry *)sfx)->getContLen()) ||
              TESTAFF(((SfxEntry *)sfx)->getCont(), (unsigned short)'%',
                      ((SfxEntry *)sfx)->getContLen()))))
        // END of LANG_hu section
        ) {
      ...
      if (langnum == LANG_hu) {
        ...
        if (pfx && (get_syllable(((PfxEntry *)pfx)->getKey(),
                                 strlen(((PfxEntry *)pfx)->getKey())) > 1))
          wordnum++;
      }
// END of LANG_hu section
      ...
      if ((rv) && (langnum == LANG_hu) && (TESTAFF(rv->astr, 'I', rv->alen)) &&
      ...
      sfx = NULL;
      sfxflag = FLAG_NULL;
      rv = (compoundflag) ? affix_check((word + i), strlen(word + i),
                                        compoundflag, IN_CPD_END)
                          : NULL;
      if (!rv && compoundend) {
        sfx = NULL;
        pfx = NULL;
      ...
      // check non_compound flag in suffix and prefix
      if ((rv) && ((pfx && ((PfxEntry *)pfx)->getCont() &&
                    TESTAFF(((PfxEntry *)pfx)->getCont(), compoundforbidflag,
                            ((PfxEntry *)pfx)->getContLen())) ||
                   (sfx && ((SfxEntry *)sfx)->getCont() &&
                    TESTAFF(((SfxEntry *)sfx)->getCont(), compoundforbidflag,
...
// check if compound word is correctly spelled
// hu_mov_rule = spec. Hungarian rule (XXX)
int AffixMgr::compound_check_morph(const char *word, int len, short wordnum,
                                   short numsyllable, short maxwordnum,
                                   short wnum, hentry **words,
                                   char hu_mov_rule = 0, char **result = NULL,
    ...
    if (((rv) && (checked_prefix || (words && words[wnum]) ||
      ...
      if (!rv && numdefcpd && words) {
        ...
        if (rv && words && defcpd_check(&words, wnum + 1, rv, NULL, 1)) {
          ...
          strcat(*result, presult);
          if (m || (*m != '\0')) {
            sprintf(*result + strlen(*result), "%c%s%s%s", MSEP_FLD, MORPH_PART,
                    word + i, line_uniq_app(&m, MSEP_REC));
          }
          if (m)
            free(m);
          strcat(*result, "\n");
...
inline int AffixMgr::isRevSubset(const char *s1, const char *end_of_s2,
                                 int len) {
...
struct hentry *AffixMgr::suffix_check(const char *word, int len, int sfxopts,
                                      AffEntry *ppfx, char **wlst, int maxSug,
                                      int *ns, const FLAG cclass,
                                      const FLAG needflag, char in_compound) {
  struct hentry *rv = NULL;
  char result[MAXLNLEN];
      ...
      if ((((in_compound != IN_CPD_BEGIN)) || // && !cclass
          ...
          (cclass ||
           ...
           (ppfx &&
            !((ep->getCont()) &&
              TESTAFF(ep->getCont(), needaffix, ep->getContLen()))))) {
        rv = sptr->checkword(word, len, sfxopts, ppfx, wlst, maxSug, ns, cclass,
      ...
      if (st) {
        if (ppfx) {
          if (((PfxEntry *)ppfx)->getMorph()) {
            ...
            debugflag(result, ((PfxEntry *)ppfx)->getFlag());
        }
        strcat(result, st);
        free(st);
        if (se->getMorph()) {
...
char *AffixMgr::suffix_check_morph(const char *word, int len, int sfxopts,
...
// check if word with affixes is correctly spelled
char *AffixMgr::affix_check_morph(const char *word, int len,
                                  const FLAG needflag, char in_compound) {
  char result[MAXLNLEN];
  char *st = NULL;
...
char *AffixMgr::morphgen(char *ts, int wl, const unsigned short *ap,
                         unsigned short al, char *morph, char *targetmorph,
                         int level) {
  // handle suffixes
  char *stemmorph;
  ...
  for (int i = 0; i < al; i++) {
    ...
    while (sptr) {
      if (sptr->getFlag() == ap[i] &&
        ...
        if (stemmorphcatpos)
          strcpy(stemmorphcatpos, sptr->getMorph());
        else
          stemmorph = (char *)sptr->getMorph();
...
int AffixMgr::expand_rootword(struct guessword *wlst, int maxn, const char *ts,
                              int wl, const unsigned short *ap,
                              unsigned short al, char *bad, int badl,
                              char *phon) {
  int nh = 0;
...
int AffixMgr::get_numrep() { return numrep; }
...
int AffixMgr::get_nummap() { return nummap; }
...
int AffixMgr::get_numbreak() { return numbreak; }
...
// return text encoding of dictionary
int AffixMgr::get_langnum() { return langnum; }

// return double prefix option
int AffixMgr::get_complexprefixes() { return complexprefixes; }

FLAG AffixMgr::get_keepcase() { return keepcase; }

int AffixMgr::get_checksharps() { return checksharps; }
...
// return the preferred try string for suggestions
const char *AffixMgr::get_wordchars() { return wordchars; }
...
FLAG AffixMgr::get_compoundflag() { return compoundflag; }

// return the forbidden words control flag
FLAG AffixMgr::get_forbiddenword() { return forbiddenword; }

// return the forbidden words control flag
FLAG AffixMgr::get_nosuggest() { return nosuggest; }

// return the forbidden words flag modify flag
FLAG AffixMgr::get_needaffix() { return needaffix; }

// return the onlyincompound flag
FLAG AffixMgr::get_onlyincompound() { return onlyincompound; }

// return the compound word signal flag
FLAG AffixMgr::get_compoundroot() { return compoundroot; }

// return the compound begin signal flag
FLAG AffixMgr::get_compoundbegin() { return compoundbegin; }

// return the value of checknum
int AffixMgr::get_checknum() { return checknum; }

// return the value of prefix
const char *AffixMgr::get_prefix() {
  if (pfx)
    return ((PfxEntry *)pfx)->getKey();
  return NULL;
}

// return the value of suffix
const char *AffixMgr::get_suffix() { return sfxappnd; }

// return the value of derived form (base word with first suffix).
const char *AffixMgr::get_derived() { return derived; }

// return the value of suffix
const char *AffixMgr::get_version() { return version; }

// return lemma_present flag
FLAG AffixMgr::get_lemma_present() { return lemma_present; }
...
// return the value of suffix
int AffixMgr::have_contclass() { return havecontclass; }

// return utf8
int AffixMgr::get_utf8() { return utf8; }

// return nosplitsugs
int AffixMgr::get_maxngramsugs(void) { return maxngramsugs; }

// return nosplitsugs
int AffixMgr::get_nosplitsugs(void) { return nosplitsugs; }

// return sugswithdots
int AffixMgr::get_sugswithdots(void) { return sugswithdots; }
  ...
  while (piece) {
    if (*piece != '\0') {
      ...
      case 1: {
        ...
        if (numrep < 1) {
          HUNSPELL_WARNING(
              stderr, "incorrect number of entries in replacement table\n");
          return 1;
        }
  ...
  for (int j = 0; j < numrep; j++) {
    ...
    while (piece) {
      if (*piece != '\0') {
        ...
        case 1: {
          reptable[j].pattern = mystrrep(mystrdup(piece), "_", " ");
          break;
        }
        case 2: {
  ...
  while (piece) {
    if (*piece != '\0') {
      ...
      case 1: {
        ...
        if (phone->num < 1) {
          HUNSPELL_WARNING(
              stderr, "incorrect number of entries in phonelacement table\n");
          return 1;
        }
  ...
  while (piece) {
    if (*piece != '\0') {
      ...
      case 1: {
        ...
        checkcpdtable =
            (replentry *)malloc(numcheckcpd * sizeof(struct replentry));
        if (!checkcpdtable)
          return 1;
        np++;
        break;
  ...
  while (piece) {
    if (*piece != '\0') {
      ...
      case 1: {
        ...
        if (numdefcpd < 1) {
          HUNSPELL_WARNING(
              stderr, "incorrect number of entries in compound rule table\n");
          return 1;
        }
...
int AffixMgr::parse_affix(char *line, const char at, FileMgr *af,
                          char *dupflags) {
...
// checking lines with bad syntax
    ...
    if (*piece != '\0') {
      ...
      case 1: {
        ...
        if (((at == 'S') && (dupflags[aflag] & dupSFX)) ||
          ...
          HUNSPELL_WARNING(
              stderr, "error: duplicate affix flag %s in line %s\n", piece, nl);
  ...
  if (np != 4) {
    ...
    if (err) {
      HUNSPELL_WARNING(
          stderr, "error: affix %s header has insufficient data in line %s\n",
          err, nl);
      free(err);
    }
    free(ptr);
  ...
  for (int j = 0; j < numents; j++) {
    ...
    while (piece) {
      if (*piece != '\0') {
        ...
        case 0: {
          ...
          if (nptr != ptr)
            nptr->opts =
                ptr->opts & (char)(aeXPRODUCT + aeUTF8 + aeALIASF + aeALIASM);
          break;
        }
        ...
        case 1: {
          ...
          if (pHMgr->decode_flag(piece) != aflag) {
            ...
            if (err) {
              HUNSPELL_WARNING(
                  stderr, "error: affix %s is corrupt near line %s\n", err, nl);
          ...
          if (dash) {
            *dash = '\0';
            ...
            if (pHMgr->is_aliasf()) {
              ...
              nptr->contclasslen =
                  (unsigned short)pHMgr->get_aliasf(index, &(nptr->contclass));
              if (!nptr->contclasslen)
                HUNSPELL_WARNING(
                    stderr, "error: bad affix flag alias: \"%s\"\n", dash + 1);
            } else {
              nptr->contclasslen = (unsigned short)pHMgr->decode_flags(
                  &(nptr->contclass), dash + 1);
    ...
    if (basefieldnum) {
      int fieldnum =
          !(nptr->morphcode) ? 5 : ((*(nptr->morphcode) == '#') ? 5 : 6);
...
int AffixMgr::redundant_condition(char ft, char *strip, int stripl,
                                  const char *cond, char *warnvar) {
  ...
  if (ft == 'P') { // prefix
    ...
    if (utf8) {
      ...
      for (i = 0, j = 0; (i < stripl) && (j < condl); i++, j++) {
        if (cond[j] != '[') {
          if (cond[j] != strip[i]) {
            HUNSPELL_WARNING(stderr, "warning: incompatible stripping "
