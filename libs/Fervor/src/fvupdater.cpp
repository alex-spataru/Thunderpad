...
void FvUpdater::InstallUpdate()
{
    ...
    // Create a new window
    m_download_dialog = new FvDownloadDialog();
    m_download_dialog->beginDownload();

    // Hide update information dialog
    hideUpdaterWindow();
}
...
void FvUpdater::startDownloadFeed (QUrl url)
{
    ...
    // The QNetworkRequest::UserAgentHeader seems to work only in Qt5...
    // however, creating the header "manually" allows the library to
    // work also in Qt4.
    //
